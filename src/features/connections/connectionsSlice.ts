...
export const testConnection = createAsyncThunk<
  ...
  string,
  {
    state: { connections: ConnectionsState };
...
>(
  ...
  async (connectionId, { getState, rejectWithValue }) => {
    ...
    const target = state.connections.items.find(
...
const connectionsSlice = createSlice({
  ...
  reducers: {
    ...
    editConnection(
      state,
      action: PayloadAction<{ id: string } & ConnectionPayload>,
    ) {
    ...
    builder.addCase(testConnection.rejected, (state, action) => {
      ...
      if (item) {
        ...
        item.lastError =
          ...
          "Credentials are not valid for this DB connection.";
      }
    });
  },
});

export const { addConnection, editConnection, removeConnection } =
